...
function App() {
  ...
  useEffect(()=>{
    if(productos.length == 0){
      fetch("https://fakestoreapi.com/products").then(res=>res.ok && res.json()).then(json=>setProductos(ajustarProductos(json))).catch(e=>setError(e));
    }
  }
  ,[])
  ...
  return (
    <>
      ...
      <div className="flex flex-row gap-[20px] justify-center items-start">
        ...
        <ListaProductos productos={productos} setProductos={setProductos} setMessage={setMessage} busqueda={busqueda} />
      </div>
      {Notification}
      {ErrorNotification}
    </>
